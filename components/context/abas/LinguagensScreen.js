...
import VoltarScreen from './VoltarScreen'; // Ajuste o caminho conforme necessário
...
const LinguagensScreen = ({ navigation }) => {
  ...
  return (
    <LinearGradient 
      ...
      <View style={styles.linguagemContainer}>
        <Image
          ...
          {linguas.map((linguagem, index) => (
            <Animatable.View 
              ...
              <TouchableOpacity
                ...
                accessible={true}
                accessibilityLabel={`Select ${linguagem.name}`}
                accessibilityHint={`Change language to ${linguagem.name}`}>
                <Image
